# Contributing to `generator-git-repo`
...
## 6. Development
`generator-git-repo` is dedicated to enabling best practices. It is therefore crucial that the generator itself follows strong engineering standards to assure quality and performance.

### 6.1. Code standards

ESLint evaluates Javascript code standards before each test run with the `npm-script pretest`.
Heres' a [table of `generator-git-repo's` ESLint rules][eslint-rules-table-url] and their enforcement.
...
[issues-new-url]: https://github.com/gregswindle/generator-git-repo/issues/new
